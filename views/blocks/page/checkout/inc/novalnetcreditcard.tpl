...
[{if $oViewConf->getActiveTheme() == 'flow'}]
    ...
    <div class="form-group novalnet_cc_new_acc" id="novalnet_cc_form" [{$displayCCForm}]>
        ...
        [{if empty($oView->getNovalnetConfig('sCreditcardDefaultLabel')) || empty($oView->getNovalnetConfig('sCreditcardDefaultInput')) || empty($oView->getNovalnetConfig('sCreditcardDefaultCss'))}]        
			[{assign var="sStyleVal" value="style='width:199%'"}]
        [{else}]
			[{assign var="sStyleVal" value=""}]
        [{/if}]
        [{if $aShoppingDetails.iShopType == '1'}]
         <div class="form-group" [{$sStyleVal}]>
...
[{block name="checkout_payment_longdesc"}]
    <div class="desc alert alert-info">
        ...
        [{if $oView->getNovalnetTestmode($sPaymentID) }]
            <br><br><span style="color:red">[{ oxmultilang ident='NOVALNET_TEST_MODE_MESSAGE' }]</span><br>
        [{/if}]

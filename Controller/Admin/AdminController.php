...
class AdminController extends \OxidEsales\Eshop\Application\Controller\Admin\ShopConfiguration
{
    ...
    private function _validateNovalnetConfig($aNovalnetConfig)
    {
        ...
        if (!empty($aNovalnetConfig['iDueDatenovalnetprepayment']) && (!is_numeric($aNovalnetConfig['iDueDatenovalnetprepayment']) || $aNovalnetConfig['iDueDatenovalnetprepayment'] < 7 || $aNovalnetConfig['iDueDatenovalnetprepayment'] > 28)) {
            $this->_aViewData['sNovalnetError'] = $oLang->translateString('NOVALNET_PREPAYMENT_INVALID_DUE_DATE_ERROR');
            return false;
        }

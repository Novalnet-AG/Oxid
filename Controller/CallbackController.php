    ...
    private function _processNovalnetCallback()
    {
        ...
        if ($iPaymentTypeLevel === 0) {
            if ($this->_aCaptureParams['payment_type'] == 'PAYPAL' && in_array($this->aOrderDetails['GATEWAY_STATUS'], [85, 90])) {
                ...
                } elseif(in_array($this->aOrderDetails['GATEWAY_STATUS'], [85, 90]) && $this->_aCaptureParams['tid_status'] == 100) {      $sTransactionConfirm = $sLineBreak .'The transaction has been confirmed on '.      date('Y-m-d H:i:s');
             ...
             } elseif (in_array($this->_aCaptureParams['payment_type'], ['PAYPAL', 'PRZELEWY24']) && $sPaymentSuccess) {
            ...
            $sNovalnetComments = 'Chargeback executed successfully for the TID: ' . $this->_aCaptureParams['tid_payment'] . ' amount ' . $dFormattedAmount . ' on ' . date('Y-m-d'). ' & '. date('H:i:s') . '. The subsequent TID: ' . $this->_aCaptureParams['tid'];
            ...
            if (in_array($this->_aCaptureParams['payment_type'], ['CREDITCARD_BOOKBACK', 'PAYPAL_BOOKBACK', 'PRZELEWY24_REFUND', 'REFUND_BY_BANK_TRANSFER_EU', 'CASHPAYMENT_REFUND', 'GUARANTEED_INVOICE_BOOKBACK', 'GUARANTEED_SEPA_BOOKBACK'])) {
                $sNovalnetComments = 'Refund has been initiated for the TID: ' . $this->_aCaptureParams['tid_payment'] . ' with the amount ' . $dFormattedAmount.' .  New TID: '.$this->_aCaptureParams['tid'].' for the refunded amount '. $dFormattedAmount;
            }
            $sComments = $sLineBreak . $sNovalnetComments;
            ...
            if (in_array($this->_aCaptureParams['payment_type'], array('INVOICE_CREDIT','CASHPAYMENT_CREDIT','ONLINE_TRANSFER_CREDIT'))) {
                if (!isset($dPaidAmount) || $dPaidAmount < $dAmount) {
                    ...
                    $sNovalnetComments = 'Credit has been successfully received for the TID: ' . $this->_aCaptureParams['tid_payment'] . ' with amount on ' . $dFormattedAmount .'. Please refer PAID transaction in our Novalnet Merchant Administration with the TID: ' . $this->_aCaptureParams['tid'];
                    $sComments = $sLineBreak . $sNovalnetComments;
                    $sUpdateSql = 'UPDATE oxorder SET NOVALNETCOMMENTS = CONCAT(IF(NOVALNETCOMMENTS IS NULL, "", NOVALNETCOMMENTS), "' . $sComments . '") WHERE OXORDERNR ="' . $this->aOrderDetails['ORDER_NO'] . '"';
                    ...
                    if ($this->_aCaptureParams['payment_type'] == 'ONLINE_TRANSFER_CREDIT') {
                        ...
                        $sTransactionComments = 'Novalnet callback script executed successfully for the TID: ' . $this->_aCaptureParams['tid_payment'] . 'with amount ' . $dFormattedAmount .' on '.  date('Y-m-d').' & '.  date('H:i:s') .'. Please refer PAID transactions in our Novalnet Merchant Administration portal with the TID: '.$this->_aCaptureParams['tid'];
